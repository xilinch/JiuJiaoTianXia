...
public class UtilIoAndr {
     ...
     * @throws IOException
     */
    public static File write2SDCard(Context context, String dirName, String fileName, String content) {
        FileOutputStream fos = null;
        try {
            File file = createFileInSDCard(context, dirName, fileName);
            if (file == null) {
                return null;
            }
    ...
    public static File createDirInAndroid(Context context, String dirName) {
        try {
            if (isSDcardExist()) {
                return createDirInSDCard(context, dirName);
            } else {
...
//        return dir;
//    }
    public static File createDirInSDCard(Context context, String dirName) {
        if (isSDcardExist()) {
            File externalCacheDir = context.getExternalCacheDir();
            if (externalCacheDir != null) {
                File packageDir = externalCacheDir.getParentFile();
                if (packageDir != null) {
                    if (UtilString.isAvaliable(dirName)) {
                        return UtilIo.createDir(packageDir.getAbsolutePath() + File.separator + dirName);
                    } else {
                        return packageDir;
                    }
                }
    ...
    public static File createFileInAndroid(Context context, String dirName, String fileName) {
        try {
            if (isSDcardExist()) {
                return createFileInSDCard(context, dirName, fileName);
            } else {
     ...
     * @throws IOException
     */
    public static File createFileInSDCard(Context context, String dirName, String fileName) {

        try {
            File dir = createDirInSDCard(context, dirName);
            if (dir == null) {
                return null;
            }
            File file = new File(dir, fileName);
